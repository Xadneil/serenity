...
#include <Userland/DevTools/DotNETRuntime/Variant.h>
...
public:
    virtual void initialize(CLIInterpreter&) { }
    virtual ~Cell() { }
...
template<>
struct AK::Formatter<DotNETRuntime::Cell> : AK::Formatter<FormatString> {
    ErrorOr<void> format(FormatBuilder& builder, DotNETRuntime::Cell const* cell)
    {
        if (!cell)
